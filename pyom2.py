...
def new_file():
    if len(textArea.get("1.0",END+'-1c'))>0:
        if messagebox.askyesno("Save","Need to save ?"):
            save()
    else:
...
def saveas():
    ...
    try:
        ...
        showerror(title="oops!", message="can't save it")



def FindInFile():
    FindString = simpledialog.askstring("Find", "Enter")
    ...
    if textdata.upper().count(FindString.upper())>0:
        ...
        label = messagebox.showinfo("Result", "Did't find it")

    print(textdata)

def exit():
    if messagebox.askyesno("Quit"," sure you want to quit?"):
        root.destroy()

#ABOUT MENU

def Aboutroot():
    label= messagebox.showinfo("About", "welcome to my world")

#HELP MENU
def helproot():
    label= messagebox.showinfo("HELP", "log on to offocial website")
